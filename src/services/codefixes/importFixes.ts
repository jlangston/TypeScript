...
namespace ts.codefix {
    ...
    registerCodeFix({
        ...
        getAllCodeActions: context => {
            ...
            let lastNewImportsModuleSpecifier: string;
                ...
                switch (fix.kind) {
                    ...
                    case ImportFixKind.AddNew: {
                        ...
                        if (!entry) {
                            ...
                            lastNewImportsModuleSpecifier = moduleSpecifier;
                        }
                        switch (importKind) {
            ...
            return createCombinedCodeActions(textChanges.ChangeTracker.with(context, changes => {
                ...
                newImports.forEach((imports, moduleSpecifier) => {
                    addNewImports(changes, sourceFile, moduleSpecifier, quotePreference, imports, /* blankLineBetween */ lastNewImportsModuleSpecifier === moduleSpecifier);
                });
            }));
        },
    });
    ...
    function addNewImports(changes: textChanges.ChangeTracker, sourceFile: SourceFile, moduleSpecifier: string, quotePreference: QuotePreference, { defaultImport, namedImports, namespaceLikeImport }: ImportsCollection, blankLineBetween: boolean): void {
        ...
        if (defaultImport !== undefined || namedImports.length) {
            insertImport(changes, sourceFile,
                makeImport(
                    ...
                    namedImports.map(n => createImportSpecifier(/*propertyName*/ undefined, createIdentifier(n))), moduleSpecifier, quotePreference), blankLineBetween);
        }
        if (namespaceLikeImport) {
            insertImport(
                ...
                createImportDeclaration(/*decorators*/ undefined, /*modifiers*/ undefined, createImportClause(/*name*/ undefined, createNamespaceImport(createIdentifier(namespaceLikeImport.name))), quotedModuleSpecifier), blankLineBetween);
        }
    }
